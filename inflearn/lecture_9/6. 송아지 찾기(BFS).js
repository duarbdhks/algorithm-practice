...
function solution (start, end) {
  ...
  while (queue.length) {
    let x = queue.shift()
    for (const nextVertex of [x + 1, x - 1, x + 5]) {
      if (nextVertex === end) return distance[x] + 1
      if (nextVertex > 0 && nextVertex <= 10000 && visited[nextVertex] === 0) {
        queue.push(nextVertex)
        distance[nextVertex] = distance[x] + 1
      }
    }
  }
  return answer;
