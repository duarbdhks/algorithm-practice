...
function solution (m, songs) {
  ...
  let max = arr.reduce(((sum, currentValue) => sum + currentValue), 0)

  while (min <= max) {
    let mid = Math.ceil((min + max) / 2)
    const dvdCnt = count(songs, mid)
    if (dvdCnt <= m) {
      max = mid - 1
      answer = mid
    } else {
...
// 요구 용량으로 song 을 구분지었을때 구분나오는 DVD 갯수
function count (songs, capacity) {
  let result = 1
  let temp = 0
  songs.forEach(songNum => {
    if ((temp + songNum) > capacity) {
      temp = 0
      result++
    }
    temp += songNum
